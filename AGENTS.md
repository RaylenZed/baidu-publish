...
4. 错误响应统一结构 `[建议]`：
   `{ "code": "TASK_TIMEOUT", "message": "...", "request_id": "...", "details": {} }`
5. 幂等接口必须接收并记录 `idempotency_key`。
6. 分页规范 `[建议]`：
   - 请求参数：`page`（从 1 开始）、`size`（默认 20，最大 100）
   - 响应结构：`{ "items": [...], "total": 150, "page": 1, "size": 20, "pages": 8 }`
...
| 模块 | 说明 | 状态 |
|---|---|---|
| 基础设施（Docker Compose 6 服务） | `nginx/backend/worker/beat/redis/postgres` | 未开始 |
| 数据库 Schema 与 Alembic | 全量表结构 + 索引 + 迁移 | 未开始 |
| 认证模块 | 登录、JWT 刷新、token 失效、登录限流 | 未开始 |
| ws_ticket 模块 | 一次性票据签发与消费 | 未开始 |
| 账号管理模块 | CRUD、Cookie 检测、导入导出 | 未开始 |
| 任务编排模块 | 创建、幂等、防重、状态流转 | 未开始 |
| Celery Worker 执行模块 | 6 步执行链路与失败重试 | 未开始 |
| 超时回收模块 | 步骤级超时 + 总超时 + 2 分钟扫描 | 未开始 |
| 任务面板模块 | 任务卡片、筛选、批量操作 | 未开始 |
| WebSocket 实时日志模块 | 日志推送、回放、鉴权 | 未开始 |
| 文章管理模块 | 列表、详情、编辑、手动发布 | 未开始 |
| partial_content 风险控制 | warning 标记、draft_only 降级、人工确认 | 未开始 |
| 定时任务模块 | CRUD、启停、Misfire 补执行 | 未开始 |
| 变量池模块 | 6 维池编辑、加权随机、组合去重提示 | 未开始 |
| 仪表盘模块 | 统计卡片、健康度、近期任务 | 未开始 |
| 系统设置模块 | 并发、限额、超时、通知配置 | 未开始 |
| 通知模块 | 企微失败通知、汇总通知、WARN 通知 | 未开始 |
| 审计模块 | `publish_attempts` + `content_events` | 未开始 |
| 安全加固模块 | Cookie 加密、导出加密、CORS/HTTPS | 未开始 |
| 系统日志模块 | 查询、筛选、自动清理 | 未开始 |
| 前端工程骨架 | 路由、布局、鉴权、API 封装 | 未开始 |
| 文档与运维脚本 | README、部署文档、环境说明 | 未开始 |
